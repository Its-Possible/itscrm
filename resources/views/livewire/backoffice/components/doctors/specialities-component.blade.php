<div class="mt-4 mb-3">
    ...
    <div id="app-doctors-specialities-selected">
        @forelse($doctorFromSpecialities as $key => $value)
            <button class="btn btn-filter speciality" wire:click="removeSpeciality('{{ $key }}')">{{ $value }} <i
                    class="ri ri-close-line"></i></button>
        @empty
            <small>Este médico não tem especialidades</small>
        @endforelse
        <input type="hidden" form="doctor-create" name="doctor-specialities" id="doctor-specialities"
               value="{{ json_encode($doctorFromSpecialities) }}"/>
    </div>
</div>
